...
object Printer {
  ...
  def apply(tree: Tree): String = {
    ...
    tree match {
      ...
      case clas: ClassDecl => {
        ...
        codeBuilder.append(apply(clas.id))
        clas.parent match {
          case Some(p) => codeBuilder.append(" " + strRep(EXTENDS) + " " + apply(p))
      ...
      case mainDecl: MainDecl => {
         codeBuilder.append(strRep(OBJECT) + " " + apply(mainDecl.obj))
       ...
       */
      case varDec: VarDecl => {
        codeBuilder.append(indent + strRep(VAR) + " " +  apply(varDec.id) + " : " + apply(varDec.tpe) + " = " + apply(varDec.expr) + ";\n")
      }
      ...
      case method: MethodDecl => {
        ...
        codeBuilder.append(strRep(DEF) + " " + apply(method.id) + "(")
      ...
      case id: Identifier => {
        try{
          if(id.getSymbol.name.equals("??")){
            codeBuilder.append(id.value+"#"+id.getSymbol.name)  
          }else{
            codeBuilder.append(id.value+"#"+id.getSymbol.id)  
          }
        }catch{
          case _ : RuntimeException => codeBuilder.append(id.value)  
        }
      ...
      case expr: ExprTree => expr match {
        ...
        case equ: Equals => codeBuilder.append("("+apply(equ.lhs) + " " + strRep(EQUALS) + " " + apply(equ.rhs)+ ")")
        ...
        case methCall: MethodCall => {
          codeBuilder.append( apply(methCall.obj) + strRep(DOT))
          codeBuilder.append( apply(methCall.meth))
          codeBuilder.append("(")
  ...
  /*
   * Find the correct String Representation
   * of a TokenKind
   */
